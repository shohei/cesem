  ...
  def summary
    ...
    if(@month.blank?)
      ...
      if (@year.blank?) 
        ...
        @requested_maintenances = @q.where(created_at: beginning_of_year..beginning_of_last_month)
        @completed_maintenances = @q.where(created_at: beginning_of_year..beginning_of_last_month).where.not(completed_at: nil)
      else
        ...
        @requested_maintenances = @q.where(created_at: beginning_of_year..beginning_of_last_month).page(params[:page]) 
        @completed_maintenances = @q.where(created_at: beginning_of_year..beginning_of_last_month).where.not(completed_at: nil)
      end
    else
      ...
      @requested_maintenances = @q.where(created_at: beginning_of_month..beginning_of_next_month).page(params[:page]) 
      @completed_maintenances = @q.where(created_at: beginning_of_month..beginning_of_next_month).where.not(completed_at: nil)
    end
